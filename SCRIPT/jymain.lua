...
function NewGame()     --选择新游戏，设置主角初始属性
  ...
  JY.Thing[202][WZ7] = JYMsgBox("请选择游戏模式", MODEXZ[1] .. "*" .. MODEXZ[2], MODEXZ2, 3, 35)
	

	--是否使用特殊人物开始游戏
  local Tp = 0
  if DrawStrBoxYesNo(-1, -1, "是否选用特殊人物进行游戏", C_WHITE, CC.DefaultFont) == true then
    local pnum = JY.Thing[202][WZ7]
    Tp = 1
    ClsN()
    local m = JYMsgBox("要选择哪位特殊人物", "【零】颖悟绝伦&苍半无双  *【水】万般皆通&百花谷主  ", TSXMLB, 2, 261)
    if m == 2 then
  ...
  --使用零二七
	if JY.Person[0]["姓名"] == JY.LEQ and Tp == 1 then
	  JY.Person[0]["生命增长"] = 8
	  JY.Person[0]["内力性质"] = 2
	...
	--使用水镜四奇
	if JY.Person[0]["姓名"] == JY.SQ and Tp == 1 then
	  JY.Person[0]["生命增长"] = 4
	  JY.Person[0]["内力性质"] = 2
	  JY.Person[0]["内力最大值"] = 100
	  JY.Person[0]["攻击力"] = 40
	...
	--使用萧雨客
	if JY.Person[0]["姓名"] == JY.XYK and Tp == 1 then
	  JY.Person[0]["生命增长"] = 6
	  JY.Person[0]["内力性质"] = 2
    ...
    if p == 0 then
	  ...
	  if r == 0 then
	      ...
	      if JY.Person[p]["武功" .. i] > 0 then
	        if p < 191 then
	          JY.Person[p]["武功等级" .. i] = 999    --BOSS武功是为极
		        ...
		        JY.Person[p]["武功等级" .. i] = JY.Thing[202][WZ7]*200 + 300 + Rnd(100)    --小兵武功为十级
		      end
		    else
		    	break;
		    end
	    ...
	    --难度生命
      if JY.Person[p]["生命最大值"] == JY.Person[p]["生命"] then		--初始满生命的才加
      ...
      --难度内力
      if JY.Person[p]["内力最大值"] == JY.Person[p]["内力"] then			--初始满内力的才加
  ...
  if JY.Person[0]["姓名"] == JY.XYK and Tp == 1 then
	...
	--SYP
  local p3 = JY.Person[0]["姓名"]
  if p3 == "SYP" then
    JY.Person[0]["姓名"] = "阿弥陀佛"
  end
  
  --非正常状态下选择特殊人物，名字变成  "阿弥陀佛" = "阿弥陀佛"
  if (p3 == JY.LEQ or p3 == JY.SQ or p3 == JY.XYK) and Tp ~= 1 then
    JY.Person[0]["姓名"] = "阿弥陀佛"
  end
  
	--如果没有外号
  if JY.Person[0]["外号"] == " " then   
    ClsN()
  ...
  while true do
    ...
    elseif keypress == VK_SPACE then
      ...
      if TFNLJS[JY.Person[id]["姓名"]] ~= nil then
        say(TFNLJS[JY.Person[id]["姓名"]], JY.Person[id]["头像代号"], 5, JY.Person[id]["姓名"])
      end
    end
...
function ShowPersonStatus_sub(id, page)
  ...
  if id == 0 and p["姓名"] == JY.SQ then
    DrawString(x1 + size * 3, y1 + h * (i), RWWH["02"], C_GOLD, size)
  end
  if id == 0 and p["姓名"] == JY.XYK then
...
function SelectThing(thing,thingnum)    
  ...
  local ynum=CC.MenuThingYnum;
	...
	local dy=(CC.ScreenH-h-2*(CC.ThingFontSize+2*CC.MenuBorderPixel+5))/2-CC.ThingFontSize*2;
...
function Game_SMap()         --场景处理主函数
    ...
    DrawSMap();
	if CC.ShowXY==1 then
        DrawString(CC.ScreenW-20*CC.FontSmall,CC.Height-20,string.format("%s %d %d",JY.Scene[JY.SubScene]["名称"],JY.Base["人X1"],JY.Base["人Y1"]) ,C_GOLD,CC.FontSmall);
	end
	JYZTB();
    ShowScreen();
    lib.SetClip(0, 0, 0, 0)
...
function ShowMenu2(menuItem,numItem,numShow,showRow,x1,y1,x2,y2,isBox,isEsc,size,color,selectColor, str)     --通用菜单函数
    local w=0;
    local h=0;   --边框的宽高
    local i,j=0,0;
    ...
    if numShow==0 or numShow > numItem then
        ...
        col=numShow;
        row = math.modf(numItem/col);
    end
    
    --local showRow = math.modf((CC.ScreenH - 40)/(size+CC.RowPixel));
    if showRow > row then
    	showRow = row;
    end
    ...
    while true do
		...
		elseif   (keyPress==VK_SPACE) or (keyPress==VK_RETURN)  then
		    ...
		    if menuItem[current][2]==nil then
		        returnValue=current;
		        break;
		    else
    ...
    if dy == 0 then
      ...
      if headid >= 0 then
        ...
        if headid > 0 and headid < 190 then
	        DrawString(xy[flag].headx + 5 + x, xy[flag].heady + 2 + y + picw-CC.Fontsmall, JY.Person[headid]["姓名"], C_GOLD, CC.Fontsmall)
	      end
...
function JYZTB()
  ...
  DrawString(15, 15+2*(CC.SmallFont+CC.RowPixel), string.format("难度:%s 周目数:%d", MODEXZ2[JY.Thing[202][WZ7]], CC.CircleNum), C_GOLD, CC.SmallFont)
end
...
--人物状态按空格的说明
TFNLJS["水笙"] = "Ｇ【天赋：似水柔情】ＨＷ在战场时敌人集气速度减三点ＰＧ【称号：冰山侠女】ＨＷ攻击时有机率造成敌人行动迟缓"
--人物状态按空格的说明
TFNLJS["李文秀"] = "Ｇ【天赋：心秀天铃】ＨＷ特系两倍成长ＰＧ【称号：白马之女】ＨＷ集气增加五点，装备白马有额外加成"
--人物状态按空格的说明
TFNLJS["石中玉"] = "Ｇ【天赋：心灵感应】ＨＷ与石破天相互攻击不会误伤，受伤时共同分担伤害ＰＧ【称号：无】ＨＷ"
...
function InputNum(str, minNum, maxNum, isEsc)
  ...
  while 1 do
    ...
    elseif key == VK_SPACE or key == VK_RETURN then
      break;
    elseif key == VK_ESCAPE and isEsc ~= nil then
      num = nil;
      break;
    end
	ClsN();
...
function SaveList()
  ...
  --读取R*.grp文件
	...
	for i=1, CC.SaveNum do
		...
		menu[i] = {string.format("%2d: %s  %s  %s  %s",i,name, lv, sname, nd), nil, 1};
		

	end

	local menux=(CC.ScreenW-14*CC.DefaultFont-2*CC.MenuBorderPixel)/2
