...
function unnamed(kfid)
  ...
  if atkarray[1].p > 0 then
    ...
    if nx ~= nil then
    lib.Debug(string.format("pid=%d, jl=%d, nx=%d, ny=%d, cx=%d, cy=%d", pid, jl, nx, ny, WAR.Person[WAR.CurID]["坐标X"], WAR.Person[WAR.CurID]["坐标Y"]));
    end
    if jl == -1 then
      AutoMove()
    else
        ...
        if step_num ~= nil then
	        ...
	        for j = 1, step_num do
	          ...
	          if Dest < 255 then
	            ...
	            array[an].x = xx
	            array[an].y = yy
	            array[an].p = jl - math.abs(nx-xx) - math.abs(ny-yy);
	          end
	        end
	      end
      end
...
function baseRandom(p)
  ...
  if inteam(p) then
    for q = 1, #TeamP do
      if TeamP[q] ~= nil and TeamP[q] == p then
...
function defRandom(jl, p)
  ...
  --防御力加成机率
  jl = jl + limitX(math.modf(def / 15), 0, 40);
...
--计算武功伤害
function War_WugongHurtLife(enemyid, wugong, level, ang)
  ...
  while true do
	...
	for i = 1, 10 do
	  ...
	  if kfid > 88 and kfid < 109 and kfid ~= 97 and WAR.Person[enemyid]["我方"] ~= WAR.Person[WAR.CurID]["我方"] then
	   
			--蛤蟆功 特效	   
	    if kfid == 95 and myrandom(20, eid) then
	      WAR.Person[enemyid]["特效文字2"] = JY.Wugong[kfid]["名称"] .. "蓄力"
        ...
        if not inteam(eid) then      --如果是敌人，气防+1000
          dng = 1000
        end
      end
		  if not myrandom(10, eid) and (eid ~= 9 or not myrandom(10 + i * 4, eid)) and (eid ~= 0 or i ~= 2 or kfid ~= 91 or not JLSD(35, 60, 0) and (GetS(4, 5, 5, 5) ~= 6 or not JLSD(30, 70, 0))) then
				
		  elseif not myrandom(10, eid) and JLSD(35, 60, eid) then
	...
	if num > 0 then
		...
		--易筋经神功护体，额外增加气防
		if kfid == 108 and atkdefRandom(25,eid) then
			dng = dng + math.modf(wl/2) + 300;	--增加气防
			WAR.L_SGHT = kfid;
		...
		--太玄神功，有机率把杀集气转为集气值
		if kfid == 102 and (JLSD(30,60,eid) or (eid == 38 and  JLSD(40,60,eid))) then
			WAR.L_SGHT = kfid;
			if WAR.Person[enemyid]["特效文字1"] ~= nil then
		...
		if kfid == 105 and (JLSD(30,60,eid) or (eid == 36 and  JLSD(40,60,eid))) and eid ~= 27 then
			WAR.L_SGHT = kfid;
			if WAR.Person[enemyid]["特效文字1"] ~= nil then
		...
		if (kfid == 106 and defRandom(30, eid)) then
			WAR.L_SGHT = kfid;
			dng = dng + 500;
			if WAR.Person[enemyid]["特效文字1"] ~= nil then
	...
	--普通主角，六如护体
	if eid == 0 and PersonKF(eid, 91) and (JLSD(25,75,eid) or (GetS(4, 5, 5, 5)==6 and JLSD(30,60,eid))) and GetS(10, 0, 7, 0) == 1 and GetS(4, 5, 5, 5) ~= 8 and WAR.Person[enemyid]["我方"] ~= WAR.Person[WAR.CurID]["我方"] then
	  local t = 0
	  local mc = JY.Wugong[91]["名称"]
	  
	  if GetS(10, 0, 11, 0) ~= 1 and JLSD(10, 60, pid) then
		  t = math.random(3)
		else
		  t = math.random(4)
		end
	  --仁者
	  if GetS(4, 5, 5, 5) == 6 and GetS(10, 0, 11, 0) ~= 1 then
	    t = math.random(2) + 1
	  end
	  ...
	  if t == 3 then
	    ...
	    if WAR.Person[enemyid]["特效文字2"] ~= nil then
	    	...
	    	WAR.Person[enemyid]["特效文字2"] = mc .. FLHSYL[2]		--其徐如林
	    end
		...
		elseif t == 2 then
		  ...
		  if WAR.Person[enemyid]["特效文字2"] ~= nil then
	    	...
	    	WAR.Person[enemyid]["特效文字2"] = mc .. FLHSYL[4]		--不动如山
	    end
		  WAR.FLHS4 = 1
		else			
		  if (GetS(4, 5, 5, 5) == 6 or math.random(10) < 5) and GetS(10, 0, 11, 0) == 1 then
		    ...
		    if WAR.Person[enemyid]["特效文字2"] ~= nil then
		    	...
		    	WAR.Person[enemyid]["特效文字2"] = mc .. FLHSYL[5]		--难知如阴
		    end
		    WAR.ACT = 10
		    WAR.ZYHB = 0
		    WAR.FLHS5 = 1
 ...
 --brolycjw: 木岛主
  ...
  --龙象般若功加力必流血
  --乔峰免流血
  if hurt > 0 and (WAR.L_YTJ1 == 1 or WAR.L_TLD == 1 or WAR.NGJL == 103 or WAR.BLX == 1 or WAR.GCTJ == 1 or  (50 < hurt and JLSD(30, 75, pid)) or (wugong == 18 and JLSD(30, 90, pid)) or WAR.L_WYJFA == 32) then
		if pid == 591 then
  ...
  --水镜四奇 云体风身 洗武功
  if 100 < WAR.YTFS and T2SQ(eid) and GetS(10, 0, 7, 0) == 1 then
    WAR.Person[enemyid]["特效动画"] = 6
    if WAR.Person[enemyid]["特效文字1"] ~= nil then
      WAR.Person[enemyid]["特效文字1"] = WAR.Person[enemyid]["特效文字1"] .. "+云体风身"
    else
      WAR.Person[enemyid]["特效文字1"] = "云体风身"
    end
    WAR.YTFS = -1
...
function WarShowHead(id)
  ...
  DrawString(x1 + 3, y1 + CC.RowPixel, "命:", C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3, y1 + CC.RowPixel + CC.Fontsmall, "内:", C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3, y1 + 2*(CC.RowPixel + CC.Fontsmall), "体:", C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3, y1 + 3*(CC.RowPixel + CC.Fontsmall), "怒气:", C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3, y1 + 4*(CC.RowPixel + CC.Fontsmall), "中毒:", C_WHITE, CC.Fontsmall)
  ...
  DrawString(x1 + 3 + 2*CC.Fontsmall, y1 + CC.RowPixel, lifexs, C_GOLD, CC.Fontsmall)
  DrawString(x1 + 3 + 2*CC.Fontsmall, y1 + CC.RowPixel + CC.Fontsmall, nlxs, C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3 + 2*CC.Fontsmall, y1 + 2*(CC.RowPixel + CC.Fontsmall), tlxs, C_WHITE, CC.Fontsmall)
  DrawString(x1 + 3 + 3*CC.Fontsmall, y1 + 3*(CC.RowPixel + CC.Fontsmall), lqzxs, C_WHITE, CC.Fontsmall)
  
  DrawString(x1 + 3 + 3*CC.Fontsmall, y1 + 4*(CC.RowPixel + CC.Fontsmall), zdxs, C_WHITE, CC.Fontsmall)		--中毒
  ...
  if WAR.Person[id]["我方"] then
  	...
  	local myx1 = 3;
  	local myy1 = 0;
  	DrawString(x1 + myx1, y1, "内伤:", C_WHITE, CC.Fontsmall)
  	DrawString(x1 + CC.Fontsmall*5/2 + myx1, y1, nsxs, C_WHITE, CC.Fontsmall)		--内伤
  	myx1 = myx1 + CC.Fontsmall * 5;
  	
  	DrawString(x1 + myx1, y1, "流血:", C_WHITE, CC.Fontsmall)
  	DrawString(x1 + myx1 + CC.Fontsmall*2 + 10, y1, lxxs, C_WHITE, CC.Fontsmall)		--流血

		myx1 = 3;
		myy1 = CC.RowPixel + CC.Fontsmall;
  	DrawString(x1 + myx1, y1  + myy1, "封穴:", C_WHITE, CC.Fontsmall)
  	DrawString(x1 + CC.Fontsmall*5/2, y1 + myy1, fxxs, C_WHITE, CC.Fontsmall)		--封穴
  	myx1 = myx1 + CC.Fontsmall * 5;
  	
		DrawString(x1 + myx1, y1  + myy1, "迟缓:", C_WHITE, CC.Fontsmall)
  	DrawString(x1 + myx1 + CC.Fontsmall*5/2, y1 + myy1, lxxs, C_WHITE, CC.Fontsmall)		--迟缓
  ...
  if WAR.Person[id]["我方"] == false then
    ...
    for i = 1, 4 do
      ...
      if wp >= 0 then
        ...
        DrawString(x1, y1 + hl * (CC.DefaultFont+CC.RowPixel), wpm .. wps, C_WHITE, CC.DefaultFont)
        hl = hl + 1
      end
    end
  end
...
function War_Fight_Sub(id, wugongnum, x, y)
  ...
  if JY.Person[pid]["左右互搏"] == 1  and WAR.ZYHB == 0 then
	  ...
	  if T2SQ(pid) and GetS(10, 0, 7, 0) == 1 then
	    zyjl = 70
	  end
  ...
  if inteam(pid) then
    for q = 1, #TeamP do
      if TeamP[q] == pid then
        local jp = math.modf(GetS(5, q, 6, 5) / 25 + 1)   --实战加连击
        if jp > 20 then
          jp = 20
        end
        LJ = LJ + jp
  ...
  while WAR.ACT <= fightnum do
    ...
    if inteam(pid) then
      for q = 1, #TeamP do
        if TeamP[q] == pid then
          local jp = math.modf(GetS(5, q, 6, 5) / 25 + 1)   --实战加成
          if jp > 20 then
            jp = 20
          end
          BJ = BJ + jp
    ...
    if WAR.BJ == 1 then
      ...
      if pid == 50 then    --乔峰
        ...
        r = math.random(3)
        if r == 1 then
          WAR.Person[id]["特效文字1"] = "教单于折箭 六军辟易 奋英雄怒"  --教单于折箭 六军辟易 奋英雄怒
        elseif r == 2 then
	    ...
	    for i = 1, 10 do
	      ...
	      if kfid == 95 then    --蛤蟆功特效
	        if WAR.tmp[200 + pid] == nil then
	          ...
	          if PersonKFJ(pid, 9) then
	          	ng = ng + ng/2;
	          	WAR.Person[id]["特效文字2"] = JY.Wugong[kfid]["名称"] .. "·神蟾震九天"
	          else
	          	WAR.Person[id]["特效文字2"] = JY.Wugong[kfid]["名称"] .. "·蟾震九天"
	          end
    ...
    --蓝烟清：新内功加力触发
		...
		for i = 1, 10 do
			...
			--罗汉伏魔功、小无相功、龙象般若功， 优先高机率触发加力
			if kfid == 96 or kfid == 98 or kfid == 103 then
				num = num + 1;
				atkjl[num] = {kfid,i};
			end
		end
		...
		if num > 0 then
			if atkRandom(30, pid) or  (pid == 0 and GetS(4, 5, 5, 5) == 6 and JLSD(30, 55, pid)) then
				...
				if WAR.Person[id]["特效文字2"] ~= nil then
					WAR.Person[id]["特效文字2"] = WAR.Person[id]["特效文字2"] .. "+" ..JY.Wugong[kfid]["名称"] .. "加力"
				else
					WAR.Person[id]["特效文字2"] = JY.Wugong[kfid]["名称"] .. "加力"
				end
		  	WAR.Person[id]["特效动画"] = math.fmod(kfid, 10) + 85
		  	WAR.NGJL = kfid
			end
    ...
    for i = 1, 10 do
      ...
      if kfid == 108 or (kfid == 107 and (JY.Person[pid]["内力性质"]==0 or (pid==0 and GetS(4, 5, 5, 5) == 5))) or kfid == 105 or kfid == 102 then
      	num = num + 1;
      	sg[num] = {kfid, i};
      end
    end
    ...
    if num > 0 then
    	...
    	--九阴神功加力，伤害额外增加20%
    	if kfid == 107 and atkRandom(25, pid) then
    		WAR.L_SGJL = kfid;
    		ng = ng + 500;
    		if WAR.Person[id]["特效文字1"] ~= nil then
    	...
    	--葵花神功加力，必造成刺目
    	if kfid == 105 and (JLSD(30,60,pid) or (pid == 36 and  JLSD(40,60,pid))) then
    		WAR.L_SGJL = kfid;
    		if WAR.Person[id]["特效文字1"] ~= nil then
    	...
    	if kfid == 102 and (JLSD(30,60,pid)  or (pid == 38 and JLSD(40,60,pid))) then
    		WAR.L_SGJL = kfid;
    		if WAR.Person[id]["特效文字1"] ~= nil then
    ...
    if wugong == 92 then
    	...
    	for j = 0, WAR.PersonNum - 1 do
        if WAR.Person[j]["我方"] ~= WAR.Person[WAR.CurID]["我方"] and WAR.Person[j]["死亡"] == false and JY.Person[WAR.Person[j]["人物编号"]]["内力"] < math.modf(nl/2) then
          f = 1;
          if pid == 13 then
    ...
    if pid == 0 and PersonKF(pid, 91) and (JLSD(25,75,0) or (GetS(4, 5, 5, 5)==6 and JLSD(30,60,0)))  and GetS(10, 0, 7, 0) == 1 and GetS(4, 5, 5, 5) ~= 8 then
      local t = 0
	  	local mc = JY.Wugong[91]["名称"]
		  if GetS(10, 0, 11, 0) ~= 1 and JLSD(10, 60, pid) then
		  	t = math.random(3)
      ...
      if GetS(4, 5, 5, 5) == 6 and GetS(10, 0, 11, 0) == 1 and JLSD(25, 75, pid) then
        t = math.random(2) + 1
      end
      if GetS(4, 5, 5, 5) == 6 and GetS(10, 0, 11, 0) == 1 and (t == 1 or t == 4) and WAR.FLHS6 > 2 then
        t = math.random(2) + 1
      end
      ...
      if t == 3 then
        ...
        if WAR.Person[id]["特效文字2"] ~= nil then
        	...
        	WAR.Person[id]["特效文字2"] = mc .. FLHSYL[1]    --其疾如风
        end
        WAR.FLHS1 = 1
        for j = 0, WAR.PersonNum - 1 do
          if WAR.Person[j]["死亡"] == false and WAR.Person[j]["我方"] == WAR.Person[WAR.CurID]["我方"] then
	    ...
	    elseif t == 2 then
	      ...
	      if WAR.Person[id]["特效文字2"] ~= nil then
        	...
        	WAR.Person[id]["特效文字2"] = mc .. FLHSYL[3]    --侵略如火
        end
	      ng = ng + 3000
	    else
	    	--仁者果然六如机率高啊
	      if (GetS(4, 5, 5, 5) == 6 or math.random(10) < 6) and GetS(10, 0, 11, 0) == 1 and WAR.FLHS6 < 3 then
	        WAR.Person[id]["特效动画"] = 6
	        if WAR.Person[id]["特效文字2"] ~= nil then
	        	WAR.Person[id]["特效文字2"] = WAR.Person[id]["特效文字2"] .. "+"..FLHSYL[6]    --动如雷震
	        else
	        	WAR.Person[id]["特效文字2"] = mc .. FLHSYL[6]    --动如雷震
	        end
	        fightnum = fightnum + 1
    ...
    --胡斐使用胡家刀法，杀集气+1200
    --[[
    if pid == 1 and JY.Person[pid]["武功" .. wugongnum] == 67 and WAR.Person[id]["特效文字3"] == nil and JY.Person[pid]["武功等级" .. wugongnum] == 999 and math.random(10) < 6 then
      WAR.Person[id]["特效文字3"] = HDZC[math.random(3)]
      WAR.WS = 1
      ng = ng + 1200
    end
    ]]
    ...
    --主角剑神大招，全屏攻击
    if pid == 0 and GetS(4, 5, 5, 5) == 2 and 120 <= JY.Person[pid]["御剑能力"] and 0 < JY.Person[pid]["武功10"] and xb == 2 and wugong ~= 49 and wugong ~= 43 and JLSD(30, 50 + pz, pid) and GetS(10, 0, 7, 0) == 1 then
      CleanWarMap(4, 0)
      for i = 0, WAR.PersonNum - 1 do
        if WAR.Person[i]["我方"] ~= WAR.Person[WAR.CurID]["我方"] and WAR.Person[i]["死亡"] == false then
          SetWarMap(WAR.Person[i]["坐标X"], WAR.Person[i]["坐标Y"], 4, 1)
    ...
    --主角拳系大招
    if pid == 0 and GetS(4, 5, 5, 5) == 1 and 0 < JY.Person[pid]["武功10"] and 120 <= JY.Person[pid]["拳掌功夫"] and JLSD(30, 50 + pz, pid) and (xb == 1 or wugong == 49) and GetS(10, 0, 7, 0) == 1 then
      WAR.Person[id]["特效动画"] = 6
      if WAR.Person[id]["特效文字3"] == nil then
    ...
    --主角特系大招
    if pid == 0 and GetS(4, 5, 5, 5) == 4 and 0 < JY.Person[pid]["武功10"] and 120 <= JY.Person[pid]["特殊兵器"] and JLSD(25, 55 + pz, pid) and xb == 4 and GetS(10, 0, 7, 0) == 1 then
      WAR.Person[id]["特效动画"] = 6
      if WAR.Person[id]["特效文字3"] == nil then
    ...
    --主角刀系大招
    if pid == 0 and GetS(4, 5, 5, 5) == 3 and 0 < JY.Person[pid]["武功10"] and 120 <= JY.Person[pid]["耍刀技巧"] and JLSD(30, 55 + pz, pid) and xb == 3 and GetS(10, 0, 7, 0) == 1 then
      WAR.Person[id]["特效动画"] = 6
      if WAR.Person[id]["特效文字3"] == nil then
	...
	if wgtype > 4 then
		...
		War_ShowFight(pid, wugong, wgtype, level, x, y, dhxg, ZHEN_ID)
	    ...
	    elseif JY.Person[pid]["武功等级" .. wugongnum] < 999 then
	      ...
	      if 999 <= JY.Person[pid]["武功等级" .. wugongnum] then
	        ...
	        if pid == 1 and wugong == 67 then
	        	say("刀法真是越练越精妙", 1);
	        	DrawStrBoxWaitKey("胡斐耍刀技巧增加10点", C_ORANGE, CC.DefaultFont)
	          ShowScreen()
    ...
    if not WAR.Person[i]["死亡"] then
      ...
      if id == 59 and GetS(86,11,11,5) == 1 then
		      ...
		      if WAR.Person[j]["人物编号"] == 0 and WAR.Person[j]["死亡"] == false and WAR.Person[j]["我方"] == WAR.Person[WAR.CurID]["我方"] then
		        for k=1, #TeamP do
		      		if TeamP[k] == id then
		      			local value = (GetS(5, k, 6, 5) + GetS(5, 1, 6, 5) - 4)/2000;			--(小龙女实战 + 主角实战) / 2000			
      ...
      if inteam(id) then
        WAR.Person[i].TimeAdd = WAR.Person[i].TimeAdd - math.modf(JY.Person[id]["受伤程度"] / 20)
      	...
      	WAR.Person[i].TimeAdd = WAR.Person[i].TimeAdd - math.modf(JY.Person[id]["受伤程度"] / 40)
      end
      ...
      if WAR.Person[i].TimeAdd > 60 then
...
--武功范围选择
function War_KfMove(movefanwei, atkfanwei)
...
for i = 0, CC.WarWidth-1 do
  ...
  --lib.PicLoadFile(CC.HeadPicFile[1], CC.HeadPicFile[2], 99, limitX(CC.ScreenW/25,12,35))
	...
	lib.LoadPNGPath(CC.MHeadPath, 99, CC.HeadNum, limitX(CC.ScreenW/25,12,35))
    ...
    for p = 0, WAR.PersonNum - 1 do
      if WAR.Person[p]["死亡"] == false and WAR.Person[p].Time > 1000 then
        ...
        if inteam(pid) and WAR.Person[p]["我方"] then
        	...
        	if WAR.L_NYZH[pid] ~= nil and math.random(10) < 7 then		--新逆运
        		r = War_Auto()
        ...
        if WAR.ZYHB == 1 then
          ...
          if PersonKF(id, 47) then
	        ...
	        if id == 0 and GetS(4, 5, 5, 5) == 7  then
	            ...
	            if WAR.HTSS == 0 and GetS(10, 0, 7, 0) == 1 and JLSD(25, 50 + pz, 0) and 0 < JY.Person[0]["武功10"] then
...
function CurIDTXDH(id, eft, MAX, str)
  ...
  for ii = starteft, emdeft do
    ...
    if str ~= nil then
      NewDrawString(-1, -1, str, C_GOLD, CC.DefaultFont)
    end
    ShowScreen()
    lib.Delay(CC.Frame)
...
function DrawTimeBar()
  ...
  local x1,x2 = CC.ScreenW * 5 / 8, CC.ScreenW * 7 / 8
  local xunhuan = true
  ...
  while xunhuan do
    for i = 0, WAR.PersonNum - 1 do
      ...
      if WAR.Person[i]["死亡"] == false then
        ...
        if PersonKF(jqid, 100) then
		   AddPersonAttrib(jqid,"内力",2 + math.random(1));
        end
        ...
        if WAR.LXZT[jqid] ~= nil then
          if inteam(jqid) then
             AddPersonAttrib(jqid,"生命", - math.random(3) - math.modf(JY.Person[jqid]["受伤程度"] / 15))
          else
             AddPersonAttrib(jqid,"生命",- 1 - math.modf(JY.Person[jqid]["受伤程度"] / 51))
          end
        ...
        if (JY.Person[jqid]["受伤程度"] > 0 and PersonKF(jqid, 108)) or (JY.Person[jqid]["受伤程度"] > 50 and (PersonKF(jqid, 106) or PersonKF(jqid, 107))) then
		...
		if jqid == 37 and GetS(86, 8, 10, 5) == 2 then
		  ...
		  if JY.Person[jqid]["受伤程度"] > 70 and JLSD(30, 70, jqid) then
			...
			--时序回内
			AddPersonAttrib(jqid,"内力",3 + math.random(2));
			
			AddPersonAttrib(jqid,"内力",1 + math.random(1));
    ...
    if math.fmod(WAR.SXTJ,3) == 1 then
...
function DrawTimeBar_sub(x1, x2, y, flag)
  ...
  for i = 0, WAR.PersonNum - 1 do
    if not WAR.Person[i]["死亡"] then
      ...
      local headid = JY.Person[WAR.Person[i]["人物编号"]]["头像代号"]
...
function myrandom(p, pp)
  ...
  if inteam(pp) then
    ...
    if T1LEQ(pp) and GetS(10, 0, 7, 0) == 1 then
      times = 2
    end
  else
    times = 3
